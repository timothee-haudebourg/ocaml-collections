...
module Make (E : OrderedType) = struct
  ...
  *)
  ...
  type enumeration = End | More of Elt.t * t * enumeration
  ...
  let rec find_last_opt f = function
    ...
    | Node{l; v; r; _} ->
      if f v then
        ...
        begin match find_last_opt f l with
          | Some v -> Some v
          | None -> find_last_opt f r
        end
  ...
  let try_join l v r =
    ...
    if (l = Empty || Elt.compare (max_elt l) v < 0)
    && (r = Empty || Elt.compare v (min_elt r) < 0)
    then join l v r
  ...
  let of_list l =
    ...
    | _ -> of_sorted_list (List.sort_uniq Elt.compare l)
  ...
  let fold_random f t x =
    let shuffle (d:Elt.t list) =
      let compare (b1, e1) (b2, e2) =
        let c = b1 - b2 in
        if c = 0 then Elt.compare e1 e2 else c
      in
