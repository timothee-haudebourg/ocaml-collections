...
sig
  ...
  type (+'a) t
  (** The type of maps from type [Key.t] to type ['a]. *)
  ...
  *)
  ...
  val merge:
    (Key.t -> 'a option -> 'b option -> 'c option) -> 'a t -> 'b t -> 'c t
  (** [merge f m1 m2] computes a map whose keys is a subset of keys of [m1]
      ...
      for any Key.t [x], provided that [f None None = None].
      @since 3.12.0
  *)
  ...
  *)
  ...
  (** [iter f m] applies [f] to all bindings in map [m].
      [f] receives the Key.t as first argument, and the associated value
  ...
  *)
  ...
  (** [split x m] returns a triple [(l, data, r)], where
        [l] is the map with all the bindings of [m] whose Key.t
      is strictly less than [x];
        [r] is the map with all the bindings of [m] whose Key.t
  ...
  *)
  ...
  (** [find_first f m], where [f] is a monotonically increasing function,
      returns the binding of [m] with the lowest Key.t [k] such that [f k],
      or raises [Not_found] if no such Key.t exists.
  ...
  *)
  ...
  (** [find_first_opt f m], where [f] is a monotonically increasing function,
      returns an option containing the binding of [m] with the lowest Key.t [k]
      such that [f k], or [None] if no such Key.t exists.
      @since 4.05
  *)

  val find_last: (Key.t -> bool) -> 'a t -> Key.t * 'a
  (** [find_last f m], where [f] is a monotonically decreasing function,
      returns the binding of [m] with the highest Key.t [k] such that [f k],
      or raises [Not_found] if no such Key.t exists.
      @since 4.05
  *)
  ...
  (** [find_last_opt f m], where [f] is a monotonically decreasing function,
      returns an option containing the binding of [m] with the highest Key.t [k]
      such that [f k], or [None] if no such Key.t exists.
      @since 4.05
  *)
  ...
  (** Same as {!Map.S.map}, but the function receives as arguments both the
      Key.t and the associated value for each binding of the map. *)
...
module Make (K : OrderedType) : S with module Key = K

module Ext (A : S) (B : S) : sig
  val map : (A.Key.t -> 'a -> B.Key.t * 'b) -> ('b -> 'b -> 'b option) -> 'a A.t -> 'b B.t

  val fold2 : (A.Key.t -> 'a -> B.Key.t -> 'b -> 'c -> 'c) -> 'a A.t -> 'b B.t -> 'c -> 'c
end

module Product (A : S) (B : S) (AB : S) : sig
  val product : (A.Key.t -> 'a -> B.Key.t -> 'b -> (AB.Key.t * 'c) option) -> 'a A.t -> 'b B.t -> 'c AB.t
end
